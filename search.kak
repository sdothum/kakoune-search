...
set-face global search_select     SecondarySelection
set-face global search_coordinate comment
...
" search %{
	...
	evaluate-commands -draft -save-regs '"' %{
		...
		evaluate-commands -no-hooks -buffer * %{
			try %{
				...
				evaluate-commands -save-regs 'c"' -itersel %{
					set-register c "%val{bufname}:%val{cursor_line}:%val{cursor_column}│⁋"  # NOTE: replaced column: with visual separator and column delimeter
		...
		edit -scratch *search*
		execute-keys R
		execute-keys "%%| column -t -s⁋ -R1 -o'  ' -l2<ret>gg"                  # ATTN: 2 column limit to avoid additional delimiter columns
		set-option buffer search_current_line 0
		add-highlighter buffer/ regex "%reg{/}" 0:search_select
		# final so that %reg{/} doesn't get highlighted in the header
		add-highlighter buffer/ regex "^([^\n]+:\d+:\d+│)" 1:search_coordinate  # NOTE: highlight source triplet as single field
		add-highlighter buffer/ line '%opt{search_current_line}' default+b
